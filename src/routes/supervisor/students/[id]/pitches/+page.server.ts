...
export const actions: Actions = {
	...
	select: async ({ request, locals, params }) => {
		...
		// Mark the chosen ones as selected with article_number 1/2.
		// Mark all other submitted pitches as rejected.
		...
		// Order chosen by their slot_index for stable article assignment.
		...
		// Updates via supabaseAdmin so we don't bump up against per-row policy
		// edge cases when transitioning multiple rows in one action.
		await supabaseAdmin
			.from('pitches')
		...
		// Reject the rest (only those that were submitted).
		await supabaseAdmin
			.from('pitches')
		...
		// Create both article rows. Article 1 starts unlocked (trigger), 2 locked.
		...
		throw redirect(303, `/supervisor/students/${params.id}`);
	}
};
