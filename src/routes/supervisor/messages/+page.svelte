	...
	{:else}
		<ul class="mt-4 space-y-2">
			{#each data.conversations as c}
				<li>
					<a href={`/supervisor/messages/${c.id}`} class="card !p-4 flex justify-between items-center hover:border-accent-blue transition-colors">
						<div>
							<div class="meta-text">{c.kind.replace('_', ' ')}</div>
							<div class="font-semibold text-sm">{c.other_name ?? 'Unknown'}</div>
							{#if c.last_body}
	...
	<div class="grid md:grid-cols-2 gap-4 mt-4">
		{#each data.students as s}
			<form method="POST" action="?/open" class="card !p-4 flex justify-between items-center">
				<div>
					<div class="meta-text">Student</div>
					<div class="font-semibold text-sm">{s.full_name ?? 'Student'}</div>
				</div>
		...
		{#each data.admins as a}
			<form method="POST" action="?/open" class="card !p-4 flex justify-between items-center">
				...
				<input type="hidden" name="kind" value="admin_supervisor" />
				<button class="btn-secondary" type="submit">Open</button>
			</form>
		{/each}
	</div>
