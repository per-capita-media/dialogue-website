	...
	{:else}
		<ul class="mt-4 space-y-2">
			{#each data.conversations as c}
				<li>
					<a href={`/student/messages/${c.id}`} class="card !p-4 flex justify-between items-center hover:border-accent-blue transition-colors">
						<div>
	...
	<div class="grid md:grid-cols-2 gap-4 mt-4">
		...
		{#each data.admins as a}
			<form method="POST" action="?/open" class="card !p-4 flex justify-between items-center">
				...
				<input type="hidden" name="kind" value="admin_student" />
				<button class="btn-secondary" type="submit">Open</button>
			</form>
		{/each}
	</div>
