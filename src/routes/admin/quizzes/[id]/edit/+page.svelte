...
<form method="POST" action="?/updateQuiz" class="card mt-6 space-y-4">
	...
	<div>
		...
		<input name="title" required class="input" value={data.quiz.title} />
	</div>
	<div class="grid md:grid-cols-2 gap-4">
		<div>
			<label class="label">Pass threshold (%)</label>
			<input name="pass_threshold" type="number" min="0" max="100" class="input" value={data.quiz.pass_threshold} />
		</div>
...
<section class="mt-10">
	...
	<form method="POST" action="?/addQuestion" class="card mt-6 space-y-3">
		...
		<div>
			...
			<input name="prompt" required class="input" />
		</div>
		<div>
			<label class="label">Options (one per line)</label>
			<textarea name="options" required rows="4" class="textarea" placeholder="Option A&#10;Option B&#10;Option C"></textarea>
		</div>
		<div>
			<label class="label">Correct option index (0-based)</label>
			<input name="correct_index" type="number" min="0" required class="input" />
		</div>
